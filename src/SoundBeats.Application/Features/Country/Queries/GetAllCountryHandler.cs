...
namespace SoundBeats.Application.Features.Country
{
    public class GetAllCountryHandler : IRequestHandler<GetAllCountryQuery, ApiResponse<MetaData<ShapedEntityDTO>>>
    {
        ...
        {
            Expression<Func<SoundBeats.Core.Entities.Country, bool>> _expressionLambda = null;
            ...
            if (!string.IsNullOrEmpty(_validFilter.Search))
            {
                ...
                _expressionLambda = _newFilter.BuildExpressionLambda<SoundBeats.Core.Entities.Country>(new BuildExpressionOptions() { ParseDatesAsUtc = false });
            }
